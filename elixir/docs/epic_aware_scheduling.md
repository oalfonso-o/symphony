...
- Issues excluded by `tracker.exclude_labels`, when the workflow configures
  exclusion labels.
- Terminal issues.
...
- Issue ids blocked because a previous session is waiting on operator input.

The contract applies to every state in `tracker.active_states`. If a workflow
configures `Ready for Agent`, `In Progress`, `Ready for Review`,
`In Review`, `Ready to Write Tests`, `Implementing Tests`,
`Ready to Integrate`, `Integrating`, or any other state as active, candidates
from that state participate in the same epic-aware ordering.
...
not automatically mutate labels or issue state for malformed candidates;
workflow agents and human intake remain responsible for enforcing issue
metadata.
...
count as active epic work. Human-only, canceled, completed, or otherwise
terminal issues do not keep an epic focused.
